...
const publishTag='<script src="/assets/example-github-publish.js?v=20260809-1"></script>';
...
for(const file of fs.readdirSync(dir).filter(name=>name.endsWith('.html'))){
  ...
  html=html.replace(/<script[^>]*src=["']\/assets\/example-github-publish\.js[^"']*["'][^>]*><\/script>\s*/gi,'');
...
console.log(`Injected deep example tools, example GitHub publishing, and practice publishing into ${count} course pages.`);
