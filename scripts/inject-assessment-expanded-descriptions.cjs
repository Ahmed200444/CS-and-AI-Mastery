...
const tag='<script defer src="../assets/assessment-expanded-descriptions.js?v=20260822-v567"></script>';
let count=0;
for(const file of fs.readdirSync(dir).filter(name=>name.endsWith('.html'))){
  const full=path.join(dir,file);
  let html=fs.readFileSync(full,'utf8');
  html=html.replace(/<script[^>]*src=["']\/assets\/assessment-expanded-descriptions\.js[^"']*["'][^>]*><\/script>\s*/gi,'');
...
console.log(`Added expanded CodeSignal-style task descriptions to ${count} course pages.`);
