...
for(const file of fs.readdirSync(dir).filter(f=>f.endsWith('.html'))){
  ...
  html=html.replace(/(<pre\b[^>]*class=(?:"[^"]*\bcode\b[^"]*"|'[^']*\bcode\b[^']*')[^>]*>)([\s\S]*?)(<\/pre>)/gi,(whole,open,body,close)=>{
    total++;
    // FIX #33/#34 -- a block that already carries an audit verdict was classified by the
    // shipped release. Keep it: re-judging native C++ examples with the Python-only
    // heuristics below downgraded 15 runnable examples to reference-only.
    if(/data-example-audit=/.test(open)){
      if(/data-example-audit="candidate"/.test(open))candidates++;else references++;
      return whole;
    }
