...
var DRAFT='csai-oa-python-draft:'+courseId+':';
...
function starter(title){return '# '+String(title||'Course exercise').replace(/\r?\n/g,' ')+'\n# Replace this starter with your solution.\n\nprint("Start your solution")\n'}
...
section.addEventListener('input',function(e){if(!e.target.matches('[data-csai-oa-python-editor]'))return;var task=e.target.closest('.oa-task'),i=Array.prototype.indexOf.call(section.querySelectorAll('.oa-task'),task);saveDraft(key(task,i),e.target.value)});
section.addEventListener('click',async function(e){
 ...
 if(e.target.closest('[data-csai-oa-publish]')){if(!ed)return;ed.focus();var publishPath='student-code/practice/'+courseId+'/'+slug(item.title||k)+'.py';if(window.CSAIMasteryGitHub&&typeof window.CSAIMasteryGitHub.publish==='function'){if(msg)msg.textContent='Publishing…';window.CSAIMasteryGitHub.publish(publishPath);setTimeout(function(){if(msg)msg.textContent='Use the GitHub status bar for publishing status.'},450)}else if(msg)msg.textContent='GitHub connection is still loading…';return}
 var btn=e.target.closest('[data-csai-oa-python-run]');if(!btn||!ed||!out)return;e.preventDefault();btn.disabled=true;btn.textContent=window.CSAIPythonRunner&&window.CSAIPythonRunner.isReady&&window.CSAIPythonRunner.isReady()?'Running Python…':'Preparing Python…';out.textContent='Preparing Python runner…';
 try{if(!window.CSAIPythonRunner||typeof window.CSAIPythonRunner.runSource!=='function')throw new Error('Python runner is still loading. Try again in a moment.');var result=await window.CSAIPythonRunner.runSource(ed.value);out.innerHTML='<span class="'+(result.error?'bad':'ok')+'">'+(result.error?'Run error':'Run complete')+'</span>\n'+esc(result.text||'(no output)')+'\n\nPython run: '+result.milliseconds+' ms';task.dispatchEvent(new CustomEvent('csai-oa-fallback-updated',{bubbles:true}))}catch(error){out.innerHTML='<span class="bad">Runner error</span>\n'+esc(error.message||String(error))}finally{btn.disabled=false;btn.textContent='▶ Run code'}
});
restore();
})();
