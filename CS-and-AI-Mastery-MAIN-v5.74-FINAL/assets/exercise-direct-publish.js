...
function languageLabel(lang){return lang==='python'?'Python':lang==='javascript'?'JavaScript':lang==='sql'?'SQL':lang==='html'?'HTML':'Other';}
function extForLanguage(lang){return lang==='python'?'py':lang==='javascript'?'js':lang==='sql'?'sql':lang==='html'?'html':'txt';}
function selectedLanguage(task){
  ...
  if(['python','javascript','sql','html'].includes(lang))return lang;
...
function editorFor(task){return task.querySelector('[data-editor]');}
...
function exercisePath(task,lang){var title=titleFor(task),ext=extension(task,lang);return 'student-code/'+languageFolder(ext)+'/'+slug(courseId)+'/'+slug(title)+'.'+ext;}
...
async function status(){var r=await fetch(STATUS_URL,{credentials:'same-origin',cache:'no-store'}),d=await r.json().catch(function(){return{};});if(!r.ok)throw new Error(d.error||'Could not check GitHub connection');if(!d.connected)throw new Error('GitHub is not connected');return d;}
...
async function publish(task,button){
  var lang=selectedLanguage(task),editor=editorFor(task),content=String(editor&&editor.value||'').trimEnd();
  if(!content){show(task,'Write your '+languageLabel(lang)+' solution first.','err');return;}
  var title=titleFor(task),ext=extension(task,lang),folder=languageFolder(ext),path=exercisePath(task,lang),old=button.textContent;
  button.disabled=true;button.textContent='Publishing…';show(task,'Publishing '+path+'…','');
  try{
    var d=await status(),repository=chooseRepo(d);if(!repository)throw new Error('Choose a repository on the GitHub page first');
...
document.addEventListener('click',function(e){var button=e.target.closest&&e.target.closest('[data-publish]');if(!button)return;var task=button.closest('.oa-task');if(!task)return;e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();publish(task,button);},true);
})();
