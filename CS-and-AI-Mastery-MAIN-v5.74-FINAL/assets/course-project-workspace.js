...
function ext(lang){return{python:'py',cpp:'cpp',javascript:'js',sql:'sql',html:'html',json:'json',shell:'sh',text:'txt'}[lang]||'txt'}
function label(lang){return{python:'Python',cpp:'C++',javascript:'JavaScript',sql:'SQL',html:'HTML',json:'JSON',shell:'Shell / config',text:'Project notes'}[lang]||lang}
function starter(lang,title){
 if(lang==='cpp')return '// '+title+'\n\n#include <iostream>\nusing namespace std;\n\nint main() {\n    cout << \"Project workspace ready\\n\";\n    return 0;\n}\n';
...
function languageOptions(current){return['python','cpp','javascript','sql','html','json','shell','text'].map(function(x){return'<option value="'+x+'" '+(x===current?'selected':'')+'>'+esc(label(x))+'</option>'}).join('')}
...
try:
    exec(compile(_SRC,'<course-project>','exec'), {'__name__':'__main__'})
except Exception:
...
async function execute(lang,code,out,p){
 ...
 if(lang==='cpp'){out.textContent='Running C++…';if(!window.CSAICppRunner||typeof window.CSAICppRunner.runSource!=='function')throw new Error('C++ runner is still loading.');var t=performance.now(),c=await window.CSAICppRunner.runSource(code);out.innerHTML='<span class="'+(c.error?'bad':'ok')+'">'+(c.error?'C++ error':'Output')+'</span>\n'+esc(c.text||'(no output)')+'\n\nC++ run: '+Math.round(performance.now()-t)+' ms';return}
