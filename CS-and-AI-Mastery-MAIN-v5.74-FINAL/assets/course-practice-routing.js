(() => {
  ...
  function scan(root){if(!document.body||document.documentElement.dataset.theme!=='dark')return;root=root&&root.nodeType===1?root:document.body;const surfaces=[];if(root.matches?.('[style*="background" i]'))surfaces.push(root);root.querySelectorAll?.('[style*="background" i]').forEach(e=>surfaces.push(e));surfaces.forEach(e=>{if(e.closest('pre,code,.code-editor,[class*="editor"],[class*="terminal"]'))return;const l=lum(rgb(getComputedStyle(e).backgroundColor));if(l!==null&&l>205&&!e.classList.contains('theme-force-surface'))e.classList.add('theme-force-surface');});const texts=[];if(root.matches?.('[style*="color" i]'))texts.push(root);root.querySelectorAll?.('[style*="color" i]').forEach(e=>texts.push(e));texts.forEach(e=>{if(e.closest('pre,code,.code-editor,[class*="editor"],[class*="terminal"]'))return;const l=lum(rgb(getComputedStyle(e).color));if(l!==null&&l<125)e.classList.add('theme-force-text');else if(l!==null&&l<175)e.classList.add('theme-force-muted');});}
  ...
  function apply(theme){const d=theme==='dark',r=document.documentElement;r.dataset.theme=theme;r.classList.toggle('dark',d);r.classList.toggle('light',!d);if(document.body){document.body.dataset.theme=theme;document.body.classList.toggle('dark',d);document.body.classList.toggle('light',!d);}localStorage.setItem(KEY,theme);localStorage.setItem('theme',theme);buttons(d);if(d)setTimeout(()=>scan(document.body),40);}
  ...
  function start(){apply(current());let darkQueue=new Set(),darkTimer=0;new MutationObserver(records=>{if(document.documentElement.dataset.theme!=='dark')return;for(const record of records)for(const node of record.addedNodes||[])if(node&&node.nodeType===1)darkQueue.add(node);if(!darkQueue.size||darkTimer)return;const flush=()=>{darkTimer=0;const batch=[...darkQueue];darkQueue.clear();batch.forEach(scan);};darkTimer=setTimeout(flush,80);}).observe(document.body,{childList:true,subtree:true});load('./full-track-lessons.js');load('./dsa-complete-course.js');load('./course-knowledge-checks.js');load('./internship-focus.js');load('./course-learning-tools.js');load('./course-metadata-panel.js');load('./github-integration.js');}
...
(() => {
  ...
  function scan(root=document){addStyles();if(root.nodeType===1&&root.matches?.('.cr-card'))enhance(root);root.querySelectorAll?.('.cr-card').forEach(enhance)}
  function start(){scan();let pending=new Set(),raf=0;new MutationObserver(records=>{for(const record of records)for(const n of record.addedNodes||[])if(n&&n.nodeType===1)pending.add(n);if(!pending.size||raf)return;raf=requestAnimationFrame(()=>{raf=0;const batch=[...pending];pending.clear();batch.forEach(scan);});}).observe(document.body,{childList:true,subtree:true});window.addEventListener('hashchange',()=>setTimeout(()=>scan(),100))}
