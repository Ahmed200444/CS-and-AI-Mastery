...
var VERSION='20260822-v567-syntax-used';
...
function explanationHtml(code,lang){var lineCount=text(code).split(/\r?\n/).length;return '<details class="csai-line-explanation" data-csai-line-explanation><summary><span>Line-by-line explanation</span><span class="csai-line-count">'+lineCount+' lines</span></summary><div class="csai-line-lazy" data-csai-line-lazy>Open this section to build the explanation.</div><div data-csai-syntax-used-host></div><div data-csai-term-host></div><ol data-csai-line-list></ol></details>';}
...
function renderLazyBlock(block){if(!block||block.dataset.csaiLineRendered==='1')return;var code=block.__csaiCode||'',lang=block.__csaiLang||inferLanguage(code,'',null),list=block.querySelector('[data-csai-line-list]'),host=block.querySelector('[data-csai-term-host]'),syntaxHost=block.querySelector('[data-csai-syntax-used-host]'),lazy=block.querySelector('[data-csai-line-lazy]');if(syntaxHost)syntaxHost.innerHTML=syntaxUsedHtml(code,lang);if(host)host.innerHTML=glossaryHtml(code,lang);if(list)list.innerHTML=listHtml(code,lang);if(lazy)lazy.remove();block.dataset.csaiLineRendered='1';}
...
window.CSAILineExplainer={version:VERSION,inferLanguage:inferLanguage,explain:explain,listHtml:listHtml,explanationHtml:explanationHtml,quickPurpose:quickPurposeFor,glossaryTerms:glossaryTerms,glossaryHtml:glossaryHtml,syntaxUsedEntries:syntaxUsedEntries,syntaxUsedHtml:syntaxUsedHtml,refresh:refreshNode,enhance:enhance,isCodeWorkspace:isCodeWorkspace,targetNodes:targetNodes};
