...
function deepText(body,heading){var heads=Array.from(body.querySelectorAll('.lesson-deep-dive h3,.lesson-main-explanation h3'));var h=heads.find(function(x){return norm(x.textContent)===norm(heading)});if(!h)return'';var out=[],n=h.nextElementSibling;while(n&&n.tagName!=='H3'){out.push(n.textContent.trim());n=n.nextElementSibling}return out.filter(Boolean).join(' ')}
...
var PY=`import sys, io, traceback, builtins
from js import window as _csai_window
_b=io.StringIO(); _old=sys.stdout; _old_input=builtins.input; sys.stdout=_b; _err=None
...
try:
    exec(compile(_SRC,'<evergreen>','exec'), {'__name__':'__main__'})
except Exception:
...
function explanation(code,lang){if(window.CSAILineExplainer&&typeof window.CSAILineExplainer.listHtml==='function')return window.CSAILineExplainer.listHtml(code,lang);return String(code||'').split(/\r?\n/).map(function(line,i){return'<li><code>'+esc(line||'⟂ blank line')+'</code><br>'+esc(explainLine(line,lang))+'</li>'}).join('')}
...
function makeCodeExample(code,lang,label,index){var run=runnable(lang,code);return'<article class="evergreen-example" data-evergreen-example data-lang="'+esc(lang)+'"><div class="evergreen-example-head"><b>'+esc(label)+'</b><span>'+esc(langLabel(lang))+'</span></div><textarea class="evergreen-editor" spellcheck="false" data-evergreen-code>'+esc(code)+'</textarea><div class="evergreen-toolbar">'+(run?'<button type="button" class="evergreen-btn primary" data-evergreen-run>▶ Run</button>':'<button type="button" class="evergreen-btn primary" data-evergreen-walk>▶ Show walkthrough</button>')+'<button type="button" class="evergreen-btn" data-evergreen-explain>Explain step by step</button><button type="button" class="evergreen-btn" data-evergreen-reset>Reset</button></div><div class="evergreen-output" data-evergreen-output>Ready.</div><div class="evergreen-explain" data-evergreen-explanation><b>Step-by-step explanation</b><ol>'+explanation(code,lang)+'</ol></div></article>'}
...
function pitfallOf(body){var n=body.querySelector('.note');if(n)return String(n.textContent||'').replace(/^Common mistake:\s*/i,'').trim();var h=Array.from(body.querySelectorAll('.lesson-main-explanation h3')).find(function(x){return /common pitfalls?/i.test(x.textContent||'')});var li=h&&h.nextElementSibling&&h.nextElementSibling.querySelector('li');return String(li&&li.textContent||'').trim()}
function buildLab(lesson,index){var body=bodyOf(lesson);if(!body||body.querySelector('[data-evergreen-lab]'))return;var lid=lessonId(lesson,index),title=titleOf(lesson),concepts=conceptsOf(body),blocks=codeBlocks(body),deep=deepText(body,'Explanation'),scenario=deepText(body,'Worked scenario'),why=deepText(body,'Why this matters');var examples=[];
 ...
 if(base){examples.push(makeCodeExample(base.code,base.lang,'Example 1 — Guided example',0));examples.push(makeScenario('Example 2 — New use case',scenario||why,concepts[1]||concepts[0]||title));examples.push(makeScenario('Example 3 — Edge case / pitfall',pitfallOf(body)||why,concepts[2]||concepts[0]||title));}
...
function bindLab(lab,title){lab.addEventListener('click',async function(e){var card=e.target.closest('[data-evergreen-example]')||e.target.closest('.evergreen-example'),out=card&&card.querySelector('[data-evergreen-output]');if(e.target.closest('[data-evergreen-run]')){var btn=e.target.closest('[data-evergreen-run]'),code=card.querySelector('[data-evergreen-code]').value,lang=card.getAttribute('data-lang');btn.disabled=true;out.textContent=lang==='python'?'Loading Python runner…':'Running…';try{await execute(lang,code,out);}catch(err){out.innerHTML='<span class="bad">Runner error</span>\n'+esc(err.message||String(err))}finally{btn.disabled=false}return}
 if(e.target.closest('[data-evergreen-walk]')){var body=lab.closest('.body'),deep=deepText(body,'Explanation'),scenario=deepText(body,'Worked scenario');out.textContent=(scenario||deep||'Explain the inputs, the transformation, and the result in your own words.');return}
...
function run(){addStyle();updateCourseStatus()}
