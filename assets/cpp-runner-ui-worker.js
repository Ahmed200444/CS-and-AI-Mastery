...
var CLIENT_URL='/assets/emception-vite/cpp-client.mjs?v=20260810-6';
var TOOLCHAIN_WORKER_URL='/assets/emception-vite/cpp-toolchain-worker.mjs?v=20260810-6';
...
async function runCode(button,variant){
 ...
 var token='cpp-'+(++seq),started=now(),wasWarm=!!orchestrator;active=token;button.disabled=true;button.textContent='Running C++…';
 try{
  ...
  var elapsed=Math.max(1,Math.round(now()-started));
  timingNote(variant,(wasWarm?'Warm C++ run: ':'C++ first run: ')+elapsed+' ms');
  variant.setAttribute('data-csai-cpp-last-ms',String(elapsed));
 }catch(error){
...
function intentTarget(node){return node&&node.closest&&node.closest('[data-lang-mode="cpp"],[data-lang-mode="dual"],[data-lang-variant="cpp"],[data-run-language-example]');}
...
document.addEventListener('click',function(event){
 var mode=event.target&&event.target.closest&&event.target.closest('[data-lang-mode="cpp"],[data-lang-mode="dual"]');
 if(mode){prewarm('language-mode');return;}
...
window.CSAICppRunner={prewarm:prewarm,isReady:function(){return!!orchestrator;},warmupMilliseconds:function(){return warmReadyAt&&warmStartedAt?Math.round(warmReadyAt-warmStartedAt):null;}};
