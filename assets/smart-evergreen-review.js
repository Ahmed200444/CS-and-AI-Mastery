...
function relatedLesson(text){var q=uniq(tokens(text)),all=lessons();if(!all.length)return null;var best=null,bestScore=-1;all.forEach(function(l,index){var score=0;q.forEach(function(t){if(l.titleTokens.indexOf(t)>=0)score+=5;if(l.conceptTokens.indexOf(t)>=0)score+=2});if(score>bestScore){bestScore=score;best=l}});if(bestScore<=0){var incomplete=all.find(function(l){var cb=l.el.querySelector('input[type="checkbox"]');return cb&&!cb.checked});return incomplete||all[0]}return best}
function taskText(node){if(!node)return'';var title=node.getAttribute('data-title')||'',h=node.querySelector('h3'),prompt=node.querySelector('.oa-prompt');return[title,h&&h.textContent,prompt&&prompt.textContent].filter(Boolean).join(' ')}
function quizText(node){if(!node)return'';var h=node.querySelector('h3');return h?h.textContent:''}
...
function reasonLabel(kind){return{reveal:'Revealed a solution',fail:'Failed a run/test',wrongQuiz:'Missed a knowledge check',evergreenFail:'Evergreen example needs work',manualAgain:'Marked Need review',manualGood:'Marked Getting it',manualEasy:'Marked Confident',success:'Solved successfully',quizCorrect:'Answered a knowledge check correctly',lessonComplete:'Completed the lesson'}[kind]||kind}
function signal(lesson,delta,kind){if(!lesson)return;var signals=read(SIGNAL_KEY);signals[courseId]=signals[courseId]||{};var s=signals[courseId][lesson.id]||{score:0,successes:0,struggles:0,reveals:0,history:[]};s.score=Math.max(-8,Math.min(12,Number(s.score||0)+delta));if(delta>0)s.struggles=(s.struggles||0)+1;if(delta<0)s.successes=(s.successes||0)+1;if(kind==='reveal')s.reveals=(s.reveals||0)+1;s.lastSignal=nowIso();s.lastReason=reasonLabel(kind);s.history=Array.isArray(s.history)?s.history:[];s.history.push({at:s.lastSignal,kind:kind,delta:delta});s.history=s.history.slice(-20);signals[courseId][lesson.id]=s;write(SIGNAL_KEY,signals);

 var review=read(REVIEW_KEY);review[courseId]=review[courseId]||{};var current=review[courseId][lesson.id]||{};var days=intervalFor(s.score);var proposed=Date.now()+days*86400000;var existing=current.nextReview?new Date(current.nextReview).getTime():Infinity;
 if(delta>0||!isFinite(existing)||existing<Date.now()){existing=proposed}else if(delta<0){existing=Math.max(existing,proposed)}
...
function inspectOutput(task,lesson,kind){var tries=0;function check(){tries++;var out=task.querySelector('[data-output],.oa-output,.quiz-feedback,.evergreen-output'),text=String(out&&out.textContent||'').toLowerCase();if(/fail|error|not quite|incorrect|check failed|0\/|1\/2|1\/3|2\/3/.test(text)){signal(lesson,3,kind||'fail');return}if(/correct|passed|run complete|query complete|check complete|output/.test(text)&&!/fail|error|not quite/.test(text)){signal(lesson,-1,kind==='wrongQuiz'?'quizCorrect':'success');return}if(tries<12)setTimeout(check,350)}setTimeout(check,250)}
...
function start(){
 ...
 document.addEventListener('click',function(e){
  var reveal=e.target.closest&&e.target.closest('[data-reveal-solution],[data-reveal],[data-solution-toggle]');if(reveal){var task=reveal.closest('[data-task],.oa-task'),lesson=nearestLessonForNode(task);signal(lesson,3,'reveal');setTimeout(showReason,30);return}
  var run=e.target.closest&&e.target.closest('[data-run],[data-evergreen-run],[data-check-quiz],[data-universal-run]');if(run){var box=run.closest('[data-task],.oa-task,[data-quiz],.quiz-card,[data-evergreen-example],.evergreen-lab')||run.parentElement,lesson=nearestLessonForNode(box);inspectOutput(box,lesson,box&&box.matches&&box.matches('[data-evergreen-example],.evergreen-lab')?'evergreenFail':'fail');return}
  var mastery=e.target.closest&&e.target.closest('[data-mastery]');if(mastery){var lab=mastery.closest('[data-evergreen-lab]'),lesson=nearestLessonForNode(lab),level=mastery.getAttribute('data-mastery');signal(lesson,level==='again'?4:level==='good'?0:-2,level==='again'?'manualAgain':level==='good'?'manualGood':'manualEasy');setTimeout(showReason,30);return}
 },true);
