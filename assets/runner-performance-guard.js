...
function loadProjectTerminal(){if(!pythonCourse()||window.__csaiProjectTerminalLoaded)return;window.__csaiProjectTerminalLoaded=true;import('/assets/python-project-terminal.js').catch(function(){window.__csaiProjectTerminalLoaded=false;});}
...
function schedule(){preconnect();if(!pythonCourse())return;loadProjectTerminal();var run=function(){if(!warmPython()){clearTimeout(timer);timer=setTimeout(run,160);}};if('requestIdleCallback' in window)window.requestIdleCallback(run,{timeout:700});else timer=setTimeout(run,180);}
