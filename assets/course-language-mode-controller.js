...
function readMode(){try{var m=localStorage.getItem(modeKey());return m==='cpp'||m==='dual'?' '+m:m==='python'?' python':' python';}catch(e){return' python';}}
function mode(){return readMode().trim();}
...
function remember(task){var ed=editor(task);if(!ed)return;var code=String(ed.value||''),lang=currentEditorLanguage(task,code);setDraft(task,lang,code);task.setAttribute('data-csai-editor-language',lang);}
...
function setFileLabel(task,m,activeLang){var label=task.querySelector('[data-file-label]');if(!label)return;var base=slug(task.getAttribute('data-title')||'exercise');label.textContent=m==='dual'?base+'.py / '+base+'.cpp':base+(activeLang==='cpp'?'.cpp':'.py');}
...
function loadLanguage(task,lang){var ed=editor(task);if(!ed)return;var current=String(ed.value||''),before=currentEditorLanguage(task,current);if(current)setDraft(task,before,current);var saved=getDraft(task,lang);if(!saved){if(before===lang&&current)saved=current;else saved=lang==='cpp'?cppStarter(task):pythonStarter();}ed.value=saved;task.setAttribute('data-csai-editor-language',lang);setDraft(task,lang,saved);}
function applyTask(task,m){var ed=editor(task);if(!ed)return;removeSecondEditor(task);var before=currentEditorLanguage(task,String(ed.value||''));if(m==='python'||m==='cpp')loadLanguage(task,m);else{if(ed.value)setDraft(task,before,ed.value);task.setAttribute('data-csai-editor-language',before);}task.setAttribute('data-csai-active-language',m);if(m!=='dual')setSelect(task,m);setFileLabel(task,m,m==='dual'?before:m);var out=task.querySelector('[data-output]');if(out&&/workspace ready|^Ready\.$/.test(out.textContent||''))out.textContent=m==='cpp'?'C++ workspace ready.':m==='dual'?'Dual mode: one editor; choose the Python or C++ publish button.':'Ready.';}
...
function queue(){clearTimeout(applyTimer);applyTimer=setTimeout(applyAll,20);}
