...
var state=load();
var RESET_ONCE='csai-oa-reset-once:'+courseId;
var justReset=false;
try{justReset=sessionStorage.getItem(RESET_ONCE)==='1';if(justReset)sessionStorage.removeItem(RESET_ONCE)}catch(e){}
...
function upgradeTask(task){
 ...
 task.addEventListener('input',function(){setTimeout(function(){updateTask(task)},0)});task.addEventListener('click',function(){setTimeout(function(){updateTask(task)},40);setTimeout(function(){updateTask(task)},900)},true);if(justReset){var ts=taskState(key);ts.results=[];ts.total=0;ts.submitted=false;var summary=task.querySelector('[data-csai-oa-summary]');if(summary){summary.textContent='Not attempted';summary.dataset.state='pending'}var box=task.querySelector('[data-csai-oa-suite] .csai-oa-tests');if(box)box.innerHTML='<div class="csai-oa-test pending"><i>•</i><span class="csai-oa-test-name">Run or submit this exercise to start a fresh OA attempt.</span><small>Reset</small></div>';}else updateTask(task);
}
...
function resetOaProgress(){
 var ok=true;
 try{ok=window.confirm('Reset OA progress for this course? This clears only the OA practice score, exercise check history, submitted-exercise count, and project assessment checks. Your lesson progress and course completion stay unchanged.')}catch(e){}
 if(!ok)return;
 try{localStorage.removeItem(STORE)}catch(e){}
 state={};
 try{sessionStorage.setItem(RESET_ONCE,'1')}catch(e){}
 location.reload();
}
function ensureDashboard(){var h=dashboardHost();if(!h||h.section.querySelector('[data-csai-oa-dashboard]'))return;var d=document.createElement('div');d.className='csai-oa-dashboard';d.setAttribute('data-csai-oa-dashboard','1');d.innerHTML='<div class="csai-oa-dashboard-main"><div class="csai-oa-kicker">Internship assessment practice</div><h3>Course Exercises · OA Mode</h3><p>Lessons stay educational and unscored. This section measures your independent exercise performance after learning.</p><div class="csai-oa-actions"><button type="button" class="csai-oa-reset-btn" data-oa-reset>Reset OA Progress</button><small>Resets only this course’s OA score/check history — lesson progress is kept.</small></div></div><div class="csai-oa-metric"><strong class="csai-oa-score" data-oa-score>— / 600</strong><small>OA Practice Score</small></div><div class="csai-oa-metric"><strong data-oa-tests>0 / 0</strong><small>Checks passed</small></div><div class="csai-oa-metric"><strong data-oa-submit>0 / 0</strong><small>Exercises submitted</small></div><div class="csai-oa-disclaimer">Training metric only — this is not an official CodeSignal score and does not reproduce the private CodeSignal scoring formula. Real unit-test results are shown only when the exercise has a machine-checkable test contract.</div>';var reset=d.querySelector('[data-oa-reset]');if(reset)reset.addEventListener('click',resetOaProgress);h.section.insertBefore(d,h.before)}
...
var timer=0;new MutationObserver(function(records){var relevant=false;for(var i=0;i<records.length&&!relevant;i++){Array.prototype.forEach.call(records[i].addedNodes||[],function(n){if(relevant||!n||n.nodeType!==1)return;if((n.matches&&n.matches('.oa-task,.project-card,.assessment-section'))||(n.querySelector&&n.querySelector('.oa-task,.project-card,.assessment-section')))relevant=true;});}if(!relevant)return;clearTimeout(timer);timer=setTimeout(boot,60)}).observe(document.documentElement,{subtree:true,childList:true});
