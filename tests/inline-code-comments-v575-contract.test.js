...
assert.ok(api&&typeof api.commentedCode==='function'&&typeof api.stripGeneratedComments==='function','commented-code and strip APIs should be available');
...
for(const line of pythonLearning.split(/\r?\n/))assert.match(line,/\s# Explanation:\s.+$/,'every nonblank Python line should end with an explanation comment');
...
assert.match(api.commentedCodeHtml('x = 1','python'),/Learning view:/,'commented view must remain available');
assert.equal(api.stripGeneratedComments(pythonLearning),pythonSource,'generated explanation comments must strip back to the clean runnable source');
